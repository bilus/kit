...
(defn- file-mismatches
  ...
  (cond
    ...
    :else (throw (ex-info "Invalid expectation type" {:expectation expectation}))))

(defn folder-mismatches
  "Compares directory contents against expectations map.
   Expectations can be:
   - string: exact content match
   - vector of regexes: all must match
   Returns map of path -> set of errors, or empty map if all match."
  {:test (fn []
  ...
  (let [actual (ls-R dir)
        ...
        content-errors (reduce (fn [acc [path expectation]]
                                 (if-let [content (get actual path)]
                                   (if-let [errors (file-mismatches content expectation)]
