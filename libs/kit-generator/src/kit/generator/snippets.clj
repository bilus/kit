...
(defn root [{:keys [project-root snippets]}]
  (io/concat-path project-root (get snippets :root "snippets")))

(defn sync-snippets! [{:keys [snippets] :as ctx}]
  (doseq [repository (-> snippets :repositories)]
    (git/sync-repository! (root ctx) repository)))
...
(defn gen-snippet [snippets-db snippet-id args]
  (when-let [{:keys [keys code]} (get-in snippets-db [snippet-id :code])]
