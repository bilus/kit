...
(let [patches [{:name "src/clj/bilus/kit_pocketbase_example/core.clj",
                :binary? false,
                :old-path
                "/Users/marcin.bilski",
                :old-text
...
(defn main-diff-view
  ...
  (let [changed-files (->> patches
    ...
    (->> changed-files
         ...
         (map :dom)
         (apply concat)
         (apply dom/div {} (toc-view changed-files)
                (dom/script {}
...
(defn write-diff-page
  [path patches]
  (spit path (main-diff-view "" "" patches)))
